...
def results(request, question_id):
    ...
    # template은 'polls/result.html'을 사용
    context = {
        'question': get_object_or_404(Question, pk=question_id)
    }
    # Template에서는 전달받은 Question인스턴스에 속하는 Choice목록을 순회하며 보여줌
    # 이 때, 각 Choice아이템들의 'choice_text' 및 'votes'속성값도 같이 출력,
    return render(request, 'polls/results.html', context)


def vote(request, question_id):
    # question_id가 pk인 Question객체를 DB로부터 가져온 데이터로 생성
    # 만약 해당하는 Question이 없다면 http404예외가 발생
    question = get_object_or_404(Question, pk=question_id)
    try:
        ...
        #       .get(pk=무언가)에서 발생 (pk에 해당하는 객체가 DB에 없을 경우)
        context = {
            'question': question,
